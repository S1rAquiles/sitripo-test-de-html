  ...
  return (
    <div className="register-container">
      <form className="register-form" onSubmit={submit}>
        <h2>Registro — Portal de Becas</h2>
        <p className="note">Crea tu cuenta con tu correo institucional</p>
        ...
        <div className="form-group">
          ...
          <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Min. 8 caracteres" required />
        </div>
        <button type="submit">Crear cuenta</button>
        {msg && <div className="error-message">{msg}</div>}
        <div className="note">Al registrarte aceptas las políticas de uso de la universidad.</div>
      </form>
    </div>
  );
}
